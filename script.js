document.addEventListener('DOMContentLoaded', function() {
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', function(e) {
      ...
      // Validate non-required email fields
      const emailFields = contactForm.querySelectorAll('input[type="email"]:not([required])');
  ...
  // Intersection Observer for better performance (fallback to scroll listener above)
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "נעלי רוקי משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            const response = await fetch('http://localhost:5001/api/email/contact-form', {
                ...
                body: JSON.stringify({
                    websiteId: '450db88b-c06a-45f3-a682-d3dd6deb6877',
                    name: data.name || '',
                    email: data.email || '',
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '450db88b-c06a-45f3-a682-d3dd6deb6877';
